document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('#new-john-form').addEventListener('submit', (event) => {
        event.preventDefault();
    })
});
...
function submit_review_edit(currentJohn) {
    latlng = {'lat': currentJohn.geometry.coordinates[0], 'lng': currentJohn.geometry.coordinates[1]};
    fetch(`/review`, {
        method: 'PUT',
        body: JSON.stringify({
...
function update_popup(currentJohnId) {
    fetch(`/fetch_data/${currentJohnId}`)
...
function new_review_modal(currentJohn) {
    ...
    document.querySelector('#submit_review').onclick = () => {
...
function open_edit_review_modal(currentJohn, currentUser) {
    ...
    currentJohn.reviews.forEach(review => {
        if (currentUser == review.username) {
            ...
            document.querySelector(`#star${review.rating}`).checked = true;
    ...
    fetch(`/fetch_data/${currentJohn.id}`)
        .then(response => response.json())
        .then(johns => {
            console.log(johns.data[0]);
        })
    
    document.getElementById('submit_review_edit').onclick = () => {
