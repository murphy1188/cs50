...
db.execute("CREATE TABLE students (f TEXT, middle TEXT, last TEXT, house TEXT, birth NUMERIC)")
...
with open(argv[1], "r") as characters:
    ...
    for row in reader:
        ...
        for name in row["name"].split():
            x = row["name"].split()
        if len(x) == 2:
            db.execute("INSERT INTO students (f, last, house, birth) VALUES(?, ?, ?, ?)", x[0], x[1], row["house"], row["birth"])

        # Insert students who have first/middle/last name
        for name in row["name"].split():
            x = row["name"].split()
        if len(x) == 3:
            db.execute("INSERT INTO students (f, middle, last, house, birth) VALUES(?, ?, ?, ?, ?)",
